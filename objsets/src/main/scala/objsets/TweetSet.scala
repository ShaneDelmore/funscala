...
abstract class TweetSet {
   ...
   */
   def union(that: TweetSet): TweetSet = {
     //def trueFilter(p: Tweet): Boolean = true;
     //filterAcc(trueFilter, that);
    //This also works and is concise but I don't like the readability.
    //It must be infering the type of tw?
   ...
   * and be implemented in the subclasses?
   */
  def mostRetweeted: Tweet //= filterAcc(tw => true, new Empty)
...
class Empty extends TweetSet {
  ...
  //def mostRetweetedAcc(acc: TweetSet): TweetSet = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  //a union implementation not based on filter.
...
object GoogleVsApple {
  ...
  //short but hard for me to read version
  lazy val googleTweets: TweetSet = allTweets.filter(tw => google.exists(tw.text.contains))
  //more explicit version
  lazy val appleTweets: TweetSet = allTweets.filter((tweet: Tweet) => apple.exists((s: String) => tweet.text.contains(s)))
